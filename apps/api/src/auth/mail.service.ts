...
`,
  ...
  async sendVerificationCode(email: string, code: string) {
    ...
    if (!apiKey) {
      ...
      console.warn('[EmailVerification] RESEND_API_KEY is not set; code:', {
        email,
        code,
      });
    ...
    const result = await resend.emails.send({
      from,
      to: email,
      subject: 'Your Fairwayd verification code',
      html: `
      <meta name="color-scheme" content="dark light">
      <meta name="supported-color-schemes" content="dark light">
      ...
      <div style="background:#0f172a;padding:28px;">
        <div style="font-family: Arial, sans-serif; line-height:1.5; max-width:560px; margin:0 auto; padding:28px; background:#111827; color:#f9fafb; border-radius:18px; border:1px solid #1f2937;">
          <div style="padding-bottom:20px; border-bottom:1px solid #374151; margin-bottom:30px;">
            <div style="display:flex; align-items:flex-end;">
              <img src="https://fairwayd-git-stage-bedachristener-cmyks-projects.vercel.app/logo.png" alt="Fairwayd" style="width:52px;height:52px;border-radius:13px;margin-right:18px;" />
              <div style="font-size:28px;font-weight:700;line-height:1;color:#f9fafb;letter-spacing:-0.4px;">Fairwayd</div>
            </div>
          </div>

          <h2 style="font-size:22px;margin:0 0 12px 0;color:#f9fafb;">Verify your email</h2>
          <p style="font-size:15px;margin:0 0 22px 0;color:#d1d5db;">Enter this code in Fairwayd to finish creating your account. It is valid for 15 minutes.</p>
          <div style="font-size:34px;letter-spacing:8px;font-weight:800;padding:16px 18px;border-radius:14px;background:#1f2937;color:#f9fafb;text-align:center;margin-bottom:22px;">${code}</div>
          <p style="font-size:13px;color:#9ca3af;margin:0;">If you did not create a Fairwayd account, you can ignore this email.</p>
        </div>
      </div>
      `,
    });
  ...
  async sendPasswordResetCode(email: string, code: string, name?: string | null) {
    const apiKey = process.env.RESEND_API_KEY?.trim();
    ...
    if (!apiKey) {
      ...
      console.warn('[PasswordReset] RESEND_API_KEY is not set; code:', {
        email,
        code,
      });
    ...
    const result = await resend.emails.send({
      from,
      to: email,
      subject: 'Reset your Fairwayd password',
      html: `
      <meta name="color-scheme" content="dark light">
      <meta name="supported-color-schemes" content="dark light">
      ...
      <div style="background:#0f172a;padding:28px;">
        <div style="font-family: Arial, sans-serif; line-height:1.5; max-width:560px; margin:0 auto; padding:28px; background:#111827; color:#f9fafb; border-radius:18px; border:1px solid #1f2937;">
          ...
          <h2 style="font-size:22px;margin:0 0 12px 0;color:#f9fafb;">Reset your password</h2>
          <p style="font-size:15px;margin:0 0 14px 0;color:#d1d5db;">${greeting}</p>
          <p style="font-size:15px;margin:0 0 22px 0;color:#d1d5db;">Enter this code in Fairwayd to reset your password. It is valid for 15 minutes.</p>
          <div style="font-size:34px;letter-spacing:8px;font-weight:800;padding:16px 18px;border-radius:14px;background:#1f2937;color:#f9fafb;text-align:center;margin-bottom:22px;">${code}</div>
          <p style="font-size:13px;color:#9ca3af;margin:0;">If you did not request a password reset, you can ignore this email.</p>
        </div>
      </div>
      `,
    });
    ...
    if (result.error) {
      console.error('[PasswordReset] Resend send failed', {
        message: result.error.message || result.error.name || 'unknown error',
        name: result.error.name,
        recipientDomain: emailDomain(email),
        fromDomain: emailDomain(from),
